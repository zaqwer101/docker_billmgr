...
# Ждём 30 секунд пока панель просрётся очень сильно
sleep 30

# Создаём дата-центр
/usr/local/mgr5/sbin/mgrctl -m billmgr datacenter.edit clicked_button=ok name=Gym name_ru="Качалка" sok=ok

# Создаём автоматический обработчик
/usr/local/mgr5/sbin/mgrctl -m billmgr processing.add.settings clicked_button=finish datacenter=1 department=3 module=pmauto name="Зал 1" sok=ok

# Создаём тарифный план
